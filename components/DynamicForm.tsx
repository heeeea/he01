...
function FieldControl({
  ...
  const baseClass =
    "mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-base font-bold text-ink outline-none transition placeholder:text-slate-400 focus:border-brand focus:ring-4 focus:ring-brand/10";
        ...
        {field.options?.map((option) => (
          <label
            ...
            className={`cursor-pointer rounded-lg border px-4 py-3 text-sm font-bold transition ${
              value === option.value ? "border-brand bg-brand text-white" : "border-slate-200 bg-white text-slate-700"
            }`}
          >
            <input
              className="sr-only"
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
      {tester.fields.map((field) => (
        <label
          key={field.id}
          className={`block rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-card backdrop-blur ${
            field.type === "textarea" ? "sm:col-span-2" : ""
          }`}
        >
          <span className="flex items-center justify-between gap-3 text-sm font-black text-ink">
            {field.label}
            {field.required ? <span className="rounded-full bg-brand/10 px-2 py-1 text-xs text-brand">必填</span> : null}
          </span>
          {field.helpText ? <span className="mt-1 block text-xs font-bold leading-5 text-slate-500">{field.helpText}</span> : null}
          <FieldControl field={field} value={values[field.id]} onChange={(value) => updateValue(field.id, value)} />
        </label>
      ))}

      <div className="sticky bottom-0 -mx-4 bg-gradient-to-t from-paper via-paper/95 to-transparent px-4 pb-4 pt-6 sm:static sm:col-span-2 sm:m-0 sm:bg-none sm:p-0">
        <div className="flex flex-col gap-3 sm:flex-row">
          <button
            type="submit"
            className="primary-button min-h-12 flex-1 text-base"
          >
            生成风险报告
            <ArrowRight size={18} />
          </button>
          <button
            ...
            className="secondary-button min-h-12 text-base"
          >
            <RotateCcw size={17} />
            重置
          </button>
